...
> 第二节中的 `Countdown(buffer, sleeper)` 只是依赖注入的一种提示，不是必须照抄的最终签名；
> 你需要根据测试和需求说明自己的选择。
> 用法：第一节看需求规格（设计驱动型，不固定契约——接口从测试中长出来）；第二节是纯任务单——
> 只给行为目标、用例表和验收命令，测试代码全部自己写；第三节是知识点讲解（按行为归置），
> 做之前通读或卡壳时查阅，做完后对照自查。
...
```text
tdd/clock/
├── countdown_test.go   # 全部测试 + Spy + synctest 对照测试
└── countdown.go        # 倒计时函数、睡眠抽象、真实睡眠实现（名字都由你定）
```
...
知识点讲解统一在第三节按行为归置，做之前通读或卡壳时查阅。

### 行为 1：输出内容正确

行为目标：倒计时依次输出 `3`、`2`、`1`、`Go!`（每个一行），写到调用方传入的输出里。
这是本包的第一个测试，也是签名的起点——`Countdown` 叫什么、收几个参数，由这个测试逼出来。

| 用例名 | 输入 | 期望断言 |
|---|---|---|
| 倒数输出 | 空 `bytes.Buffer` | `buffer.String()` 恰为 `"3\n2\n1\nGo!\n"` |

先写测试（构造 buffer、调用、断言内容）：跑 `go test ./tdd/clock` 看到编译失败即 RED；
再写最少实现变绿（先别管"等 1 秒"，那是行为 2 的事）。
`bytes.Buffer` 怎么用、参数类型怎么选，卡壳时查第三节「行为 1」。
...
把测试改成：注入一个 Spy 代替真实睡眠，断言"睡了几次"。Spy 叫什么、长什么样是你的设计——
它需要能记录"被调了几次"（卡壳时查第三节「行为 2」，里面有最小示例）。

| 用例名 | 输入 | 期望断言 |
|---|---|---|
| 每步之间睡一次 | Spy 注入倒计时 | Spy 的调用次数 == 3（写完 3、2、1 后各睡一次，`Go!` 之后不睡） |

流程提示：这一步要把 `Countdown(buffer)` 改成 `Countdown(buffer, sleeper)`——
已有测试编译不过，同样算 RED。测试里做三件事：构造 Spy、把 Spy 传进去、断言调用次数。
跑 `go test ./tdd/clock` 确认变绿。
...
思路提示：Spy 需要截获"写"——让它不仅是 Sleeper，还兼任 Writer。测试里把**同一个 Spy**
同时作为输出目标和 Sleeper 传进 `Countdown`（具体写法见第三节「行为 3」的示例）。

断言：期望序列是 `[write, sleep, write, sleep, write, sleep, write]`——4 次写、3 次睡，
严格交替。测试代码自己写，写法和行为 2 的断言同构；切片怎么比较，查第三节「行为 3」。
跑 `go test ./tdd/clock` 确认变绿。
...
（这一步的取舍与纪律——为什么不用包级变量替换做替身、已有测试能不能动——见第三节「REFACTOR」。）
...
气泡是什么、API 怎么用，先读第三节「行为 4」（有完整用法示例），再回来自己写。
...
测试代码和断言辅助函数（如 `assertOutput(t, buffer, want)`）全部自己写。
跑 `go test ./tdd/clock -v` 应看到 synctest 版瞬间跑完；再跑 `go test ./tdd/clock -race`
确认无数据竞争。
...
知识点按行为归置：做之前通读，或卡壳时按行为查阅；速查表与书目对应保留在最后。
...
### 行为 3：顺序也要对——先写 3 再睡，再写 2……

兼任 Sleeper 和 Writer 的 Spy 示例：
...
### 行为 4（进阶对照）：用 `testing/synctest` 重写

用法示例（断言辅助函数 `assertOutput(t, buffer, want)` 自己写）：
...
3. **测试函数自己也是气泡成员**：测试里写 `time.Sleep(1 * time.Second)` 不是真睡，是主动把
   假时钟往前拧 1 秒。第二节时刻表里"前进 1 秒后"读作：测试的 Sleep 返回时，气泡时钟恰好
   走过 1 秒，倒计时 goroutine 的定时器已触发、`2` 已写出。
4. **`synctest.Wait()`**：阻塞到"气泡里除自己外的 goroutine 全部持久阻塞"。没有它，
   ...
   （不 flaky）的原因，也是上面的示例里每次推进时间后都补一个 `Wait` 再断言的道理。
