...
```text
测试代码 ──► Wallet.Deposit / Balance / Withdraw    存、查、取
fmt 包 ──► Bitcoin.String()                         %v/%s 打印、t.Errorf 失败输出时自动回调
```

注意第二行：`String` 不是测试直接调用的，而是 `fmt` 包按 Stringer 接口回头调用的——
行为 2 的机制实验就是让你亲眼看到这次回调发生。
...
```text
tdd/wallet/
├── wallet_test.go    # 全部测试（含断言辅助函数）
└── wallet.go         # Bitcoin、Wallet、ErrInsufficientFunds 及全部方法
```

| # | 文件 | 这个文件是干什么的 | 里面要写的符号 | 什么时候建 |
|---|---|---|---|---|
| 1 | `wallet_test.go` | 覆盖行为 1、2、3 的全部测试 + 两个断言辅助函数 | `TestWallet` 等三个行为的测试函数（命名自取）、`assertBalance`、`assertError` | **第 1 个建** |
| 2 | `wallet.go` | 钱包实现：自定义类型、钱包及方法、哨兵错误 | `Bitcoin`、`Bitcoin.String`、`Wallet`、`Wallet.Deposit`、`Wallet.Balance`、`Wallet.Withdraw`、`ErrInsufficientFunds` | 测试编译报错时 |

要写的函数/方法一共 4 个，就是下面契约里的全部，一个不多一个不少。

### 接口契约（固定，按此实现，名字不要改）

完备性原则：**你要写的每一个类型、每一个签名都在下面**。本练习只有一个实现文件，
这份契约就是 `wallet.go` 的全部内容（需要 `import "errors"` 和 `"fmt"`）。
...
// 带类型的 int 不能直接当 Bitcoin 传入；无类型整数常量仍可按赋值规则转换。
type Bitcoin int
...
- 2 个类型：`Bitcoin`、`Wallet`
- 4 个函数/方法：`Bitcoin.String`、`Wallet.Deposit`、`Wallet.Balance`、`Wallet.Withdraw`
- 1 个哨兵错误：`ErrInsufficientFunds`
...
先写测试：跑 `go test ./tdd/wallet` 看到 `undefined: Wallet` 的编译失败即 RED；
再写最少实现（本步只需 `Bitcoin`、`Wallet`、`Deposit`、`Balance`；`String()` 留到行为 2 再写，这是故意的）变绿。
做完下面的机制实验再往下走。
...
步骤：自己写测试 → 跑 `go test ./tdd/wallet` 看到 `undefined: Withdraw`（RED）→
写最小实现到绿 → **REFACTOR**：把两个测试里重复的余额断言提取成 `assertBalance` 辅助函数
（别忘了在辅助函数里调 `t.Helper()`，原因见第三节行为 2 知识点）。
...
先写测试（错误断言同样值得提取成 `assertError` 辅助函数复用），编译失败即 RED，再写最少实现变绿。
...
3. **值接收者还是指针接收者（uber·receiver）**：要修改字段 → 必须指针；只读但结构体较大 → 指针（避免整份拷贝）；只读且小（基本类型包装、小 struct）→ 值也可以；含 `sync.Mutex` 等同步字段 → 必须指针（同步值首次使用后不得复制，`go vet` 可检查）。最后一条团队规则：**同一个类型的方法，接收者风格要统一**——所以只读的 `Balance()` 在契约里也是指针接收者，与 `Deposit`/`Withdraw` 保持一致，不让调用方猜。
4. **自定义类型 `type Bitcoin int`**：不是类型别名（别名写作 `type Bitcoin = int`，那只是同一个类型的另一个名字）。`Bitcoin` 是以 `int` 为底层类型的**全新类型**：带类型的 `int` 值不能隐式当 `Bitcoin` 用，必须显式转换；但无类型整数常量（如 `10`）可以按赋值规则直接传入。收益是编译器能阻止许多有类型单位混用，而不是阻止所有数字字面量混用。
...
2. **`String()` 里的无限递归陷阱**：实现里若写 `fmt.Sprintf("%s BTC", b)`，`%s` 又会回头调 `b.String()`——无限递归直到栈溢出。`%d` 之所以安全：`fmt` 只在字符串类动词上查 Stringer，`%d` 走整数格式化路径，不再回调。`go vet` 主要检查格式动词与参数类型是否匹配，不能可靠证明不存在这种运行时递归；递归风险要靠代码审查和专门实验验证。
...
| 含 `sync.Mutex` 等同步字段 | 必须指针 | 同步值首次使用后不得复制，`go vet` 可检查 |
...
- `%d` 不查 Stringer → `String()` 内部格式化自己必须用 `%d`；用 `%s` 可能无限递归（`go vet` 不保证能静态发现）
...
go vet ./tdd/wallet            # 无格式检查警告；递归实验需单独运行测试观察
