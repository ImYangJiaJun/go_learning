...
| 文件 | 里面写什么 | 什么时候建 |
|---|---|---|
| `dict_test.go` | 行为 1~5 的全部测试 | **第 1 个建** |
| `dict.go` | `Dict` 的 6 个方法 + `Len` | 测试编译报错时 |
| `bench_test.go` | 行为 6 的 4 个 benchmark | 行为 6 开始时 |
| `rwmutexmap.go` | `RWMutexMap`（含构造函数） | benchmark 编译报错时 |
| `counter.go` | `AtomicCounter` / `MutexCounter` | 同上 |
...
1. 在 `tdd/syncmap/` 下新建 `dict_test.go`，写入：
...
func TestDictStoreLoad(t *testing.T) {
	var d Dict // 零值即可用：没有 NewDict()，也不需要

	d.Store("lang", "go")

	// 存在的键：返回 (值, true)——和普通 map 的 v, ok := m[k] 是同一套习惯
	got, ok := d.Load("lang")
	if !ok {
		t.Fatal("键 lang 已存入，ok 应为 true")
	}
	if got != "go" {
		t.Errorf("期望 %q，得到 %v", "go", got)
	}

	// 不存在的键：返回 (nil, false)
	got, ok = d.Load("missing")
	if ok {
		t.Error("键 missing 不存在，ok 应为 false")
	}
	if got != nil {
		t.Errorf("不存在的键应返回 nil，得到 %v", got)
	}
}
```

2. 运行 `go test ./tdd/syncmap` → **编译失败**：`undefined: Dict`。
   这就是 RED——测试描述了你想要但还不存在的代码。
3. 新建 `dict.go`，照契约写出让测试通过的**最少代码**：
   `Dict` 内嵌 `sync.Map`，`Store`/`Load` 各一行委托。`Len` 等其余方法
   可以等对应行为的测试编译报错时再补。
4. 再跑 `go test ./tdd/syncmap` → 绿，行为 1 完成。
...
每个行为 = 一轮完整的 RED → GREEN，**先把测试写出来再实现**（行为 1 已在第一节完成）。
...
3. **非一致快照**：Range 期间别的 goroutine 增删，结果可能看到也可能看不到，但同一个键不会被访问两次。并发代码里不要依赖 Range 的"实时性"。（实现上 Range 开始前可能先做一次 dirty 提升，第三节讲。）
4. **Delete 是标记删除**：Delete 后 `Load` 立刻失败——逻辑上已删；但内部条目可能还在内存里躺着，等批量清理，所以 Delete 很便宜。机制在第三节讲透，这里先记住结论。
...
骨架直接给你（并发测试的骨架本身就是知识点），断言行留空——自己补：
...
2. **WaitGroup 三步铁律**：`Add` 必须在 goroutine 启动**之前**（循环内先 `Add(1)` 再 `go`，或循环外一次 `Add(10)`）；goroutine 里 `defer wg.Done()`；主 goroutine `wg.Wait()`。把 `Add` 放进 goroutine 体内是经典 bug——`Wait` 可能先于 `Add` 执行直接返回，测试失去等待。
3. **循环变量与闭包**：Go 1.22 起 for 循环变量每次迭代都是新变量，闭包直接捕获 `g` 也不会串号；骨架里仍用 `go func(g int)` 显式传参——意图更清晰，也是旧代码里的常见写法，两种都要认得。
...
新建一个临时测试文件，把上面的测试改成裸 `map[string]int`（不加任何锁），跑 `-race`：
...
1. **`b.RunParallel`**：默认起 GOMAXPROCS 个 goroutine 并行跑被测体，每个 goroutine 各跑自己的 `for pb.Next()` 循环——专门测并发吞吐和锁竞争。串行的 `b.N` 循环测不出竞争：锁没人抢，永远显得很快。
...
| 单个计数器 / 标志位 | `atomic.Int64` 等 | 一条 CPU 指令，无锁无阻塞 |
